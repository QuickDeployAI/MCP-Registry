 ...
 * `_fetchedAt`) in LanceDB for semantic search, plus an in-memory index for
 * fast structured queries.
 *
 * Embedding text is built from all top-level string values in the item.
 *
...
/** Build a search text string from all top-level string values in the item. */
function buildSearchText(item: Record<string, unknown>): string {
...
export class VectorStore<TItem extends NativeItem = NativeItem> implements StoreAdapter<TItem> {
  ...
  async load(feedUrl: string): Promise<void> {
    const metaPath = join(this.storagePath, feedDirName(feedUrl), "meta.json");
    ...
    try {
      ...
      const stored = rows
        .map((r) => { try { return JSON.parse(r.payload) as StoredItem<TItem>; } catch { return null; } })
        .filter((i): i is StoredItem<TItem> => i !== null);
      if (stored.length > 0) await this.memory.restoreItems(feedUrl, stored);
  ...
  async ingest(feedUrl: string, incoming: TItem[]): Promise<number> {
    ...
    if (newCount > 0 && this.embedder) {
      const table = await this.getOrCreateTable(feedUrl);
      const newItems = (await this.memory.getAllItems(feedUrl)).filter((i) => !before.has(i._id));
      ...
      for (const item of newItems) {
        ...
        rows.push({ id: item._id, feedUrl, vector, payload: JSON.stringify(item) });
      }
  ...
  async updatePollingMeta(feedUrl: string, pollingEnabled: boolean, pollIntervalMs: number | null): Promise<void> {
  ...
  async searchItems(feedUrl: string, query: string, topK: number): Promise<StoredItem<TItem>[] | undefined> {
